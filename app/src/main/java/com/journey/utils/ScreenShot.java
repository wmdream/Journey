...
public class ScreenShot {
    ...
    public static void shoot(Activity a) {
        ScreenShot.savePic(ScreenShot.takeScreenShot(a), "/sdcard/screen_test.jpg");
        // log.i("shoot(Activity a)");
    }

    // 程序入口2
    public static void shootView(View view) {
        ScreenShot.savePic(ScreenShot.convertViewToBitmap(view), "sdcard/xx.jpg");
    ...
    public String getSDCardPath(Context context) {
        ...
        if (!file.exists()) {
            file.mkdirs();
            //MyApplication.htlog.i("mkdirs");
        }

        if (sdcardDir == null) {
            Toast.makeText(context, "没有查找到SD卡", Toast.LENGTH_SHORT).show();
            return "";
        } else {
